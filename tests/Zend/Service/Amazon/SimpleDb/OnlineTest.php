    ...
    public function request($method, $args = [])
    {
        ...
        for ($try = 1; $try <= $this->_testWaitRetries; $try++) {
            try {
                ...
                if (!str_contains($e->getMessage(), 'currently unavailable')) {
                    throw $e;
                }
            }
        }
    ...
    public function testListDomains() {
        ...
        try {
            ...
            if (!$isLast) {
              ...
              $token = $page->getToken();
              if ($token) {
                $tokenDomainName = base64_decode((string) $token);
                if (str_contains($tokenDomainName, (string) $this->_testDomainNamePrefix)) {
                  try {
