...
class Zend_EventManager_EventManagerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $events = new Zend_EventManager_EventManager([self::class, static::class]);
        $identifiers = $events->getIdentifiers();
        $this->assertSame(count($identifiers), 1);
        $this->assertSame($identifiers[0], self::class);
     ...
     */
    public function testInvalidArgumentExceptionCanBeThrown(): never
    {
    ...
    {
        $string = $e->getParam('string', $this->default);
        return str_rot13((string) $string);
    }
    ...
    public function stringPosition($e)
    {
        ...
        return strpos((string) $string, (string) $search);
    }
    ...
    public function stringInString($e)
    {
        $string = $e->getParam('string', '');
        $search = $e->getParam('search', '?');
        return strstr((string) $string, (string) $search);
    }
