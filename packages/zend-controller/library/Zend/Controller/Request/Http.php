...
class Zend_Controller_Request_Http extends Zend_Controller_Request_Abstract
{
    ...
    {
        return match (true) {
            isset($this->_params[$key]) => $this->_params[$key],
            isset($_GET[$key]) => $_GET[$key],
            isset($_POST[$key]) => $_POST[$key],
            isset($_COOKIE[$key]) => $_COOKIE[$key],
            $key == 'REQUEST_URI' => $this->getRequestUri(),
            $key == 'PATH_INFO' => $this->getPathInfo(),
    ...
    {
        return match (true) {
            isset($this->_params[$key]) => true,
            isset($_GET[$key]) => true,
            isset($_POST[$key]) => true,
            isset($_COOKIE[$key]) => true,
            isset($_SERVER[$key]) => true,
            isset($_ENV[$key]) => true,
            default => false,
        };
    }
    ...
    public function setRequestUri($requestUri = null)
    {
        if ($requestUri === null) {
            if (
                ...
                if (str_starts_with((string) $requestUri, $schemeAndHttpHost)) {
                    $requestUri = substr((string) $requestUri, strlen($schemeAndHttpHost));
                }
    ...
    public function setBaseUrl($baseUrl = null)
    {
        ...
        if ($baseUrl === null) {
            $filename = (isset($_SERVER['SCRIPT_FILENAME'])) ? basename((string) $_SERVER['SCRIPT_FILENAME']) : '';

            if (isset($_SERVER['SCRIPT_NAME']) && basename((string) $_SERVER['SCRIPT_NAME']) === $filename) {
                $baseUrl = $_SERVER['SCRIPT_NAME'];
            } elseif (isset($_SERVER['PHP_SELF']) && basename((string) $_SERVER['PHP_SELF']) === $filename) {
                $baseUrl = $_SERVER['PHP_SELF'];
            } elseif (isset($_SERVER['ORIG_SCRIPT_NAME']) && basename((string) $_SERVER['ORIG_SCRIPT_NAME']) === $filename) {
            ...
            if (str_starts_with($requestUri, dirname((string) $baseUrl))) {
                // directory portion of $baseUrl matches
                $this->_baseUrl = rtrim(dirname((string) $baseUrl), '/');
                return $this;
            }
            ...
            if ((strlen($requestUri) >= strlen((string) $baseUrl))
    ...
    public function setPathInfo($pathInfo = null)
    {
        if ($pathInfo === null) {
            ...
            if (!empty($baseUrl) || !empty($baseUrlRaw)) {
                ...
                } elseif (str_starts_with($requestUri, $baseUrlRaw)) {
                    $pathInfo = substr($requestUri, strlen($baseUrlRaw));
                } elseif (str_starts_with($requestUri, $baseUrlEncoded)) {
