...
abstract class Zend_Db_Table_Abstract
{
    ...
    public function setDefaultValues(Array $defaultValues)
    {
        foreach ($defaultValues as $defaultName => $defaultValue) {
            if (array_key_exists((string) $defaultName, $this->_metadata)) {
    ...
    protected function _setupDatabaseAdapter()
    {
        if (! $this->_db) {
            ...
            if (!$this->_db instanceof Zend_Db_Adapter_Abstract) {
                // require_once 'Zend/Db/Table/Exception.php';
                throw new Zend_Db_Table_Exception('No adapter found for ' . static::class);
            }
        }
    }
    ...
    public function insert(array $data)
    {
         ...
         */
        if (array_key_exists((string) $pkIdentity, $data)) {
            ...
            if (count($resultSet) > 0 ) {
                foreach ($resultSet as $row) {
                    ...
                    foreach ($depTables as $tableClass) {
                        ...
                        $t->_cascadeDelete(
                            static::class, $row->getPrimaryKey()
                        );
                    }
                }
            }
     ...
     */
    #[\Deprecated(message: 'since 0.9.3 - use createRow() instead.')]
    ...
    public function createRow(array $data = [], $defaultSource = null)
    {
        ...
        if ($defaultSource == self::DEFAULT_DB) {
            ...
            foreach ($this->_defaultValues as $defaultName => $defaultValue) {
                if (array_key_exists((string) $defaultName, $defaults)) {
