 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Db_Select implements \Stringable
{
    ...
    public function group($spec)
    {
        ...
        foreach ($spec as $val) {
            ...
            if (preg_match(self::REGEX_COLUMN_EXPR_GROUP, (string) $noComments)) {
    ...
    public function order($spec)
    {
        ...
        foreach ($spec as $val) {
            if ($val instanceof Zend_Db_Expr) {
                ...
                if (preg_match('/(.*\W)(' . self::SQL_ASC . '|' . self::SQL_DESC . ')\b/si', (string) $val, $matches)) {
                    $val = trim($matches[1]);
                    $direction = $matches[2];
                }
                // Remove comments from SQL statement
                $noComments = preg_replace(self::REGEX_SQL_COMMENTS, '$1', (string) $val);
                if (preg_match(self::REGEX_COLUMN_EXPR_ORDER, (string) $noComments)) {
                    $val = new Zend_Db_Expr($val);
                }
    ...
    public function assemble()
    {
        ...
        foreach (array_keys(self::$_partsInit) as $part) {
            $method = '_render' . ucfirst((string) $part);
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        ...
        if (!is_object($tableName) && str_contains((string) $tableName, '.')) {
            [$schema, $tableName] = explode('.', (string) $tableName);
        }
    ...
    private function _uniqueCorrelation($name)
    {
        ...
        for ($i = 2; array_key_exists((string) $c, $this->_parts[self::FROM]); ++$i) {
            $c = $name . '_' . (string) $i;
        }
        return $c;
    }
